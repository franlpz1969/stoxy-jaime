    ...
    <div className="pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-between items-center py-4 mb-2 px-4 relative z-50">
        <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={menuRef}>
                <button 
                    ...
                    className="flex items-center gap-2 hover:bg-zinc-900 p-2 rounded-xl transition-colors -ml-2"
                >
                    <h1 className="text-2xl font-bold tracking-tight">{activePortfolioName}</h1>
                    <ChevronDown size={20} className={`text-zinc-500 transition-transform duration-300 ${isPortfolioMenuOpen ? 'rotate-180' : ''}`} />
                </button>

                {isPortfolioMenuOpen && (
                    <div className="absolute top-full left-0 mt-2 w-72 bg-[#1C1C1E] border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-[100]">
                        <div className="p-2 space-y-1 max-h-[300px] overflow-y-auto custom-scrollbar">
                            <div className="px-3 py-2 text-xs font-bold text-zinc-500 uppercase">Switch Portfolio</div>
                            {portfolios.map(p => (
                                <div key={p.id} className={`group flex items-center justify-between w-full rounded-xl transition-colors ${activePortfolioId === p.id ? 'bg-blue-600/10' : 'hover:bg-zinc-800'}`}>
                                    <button
                                        onClick={() => {
                                            if(onSwitchPortfolio) onSwitchPortfolio(p.id);
                                            setIsPortfolioMenuOpen(false);
                                        }}
                                        className={`flex-1 text-left px-3 py-3 flex items-center justify-between ${activePortfolioId === p.id ? 'text-blue-500' : 'text-zinc-300'}`}
                                    >
        ...
        <button 
           ...
           className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 text-blue-400 hover:text-white hover:border-blue-500 transition-all flex items-center justify-center group shadow-lg"
      ...
      <div className="px-4 mb-8">
        <div className="bg-gradient-to-br from-[#1e1e24] to-[#121214] border border-white/5 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                        <span className="text-zinc-400 text-sm font-semibold tracking-wider uppercase">{t('Total Balance', 'Balance Total')}</span>
                        <button 
                    ...
                    <div className="relative" ref={currencyMenuRef}>
                        <button 
                            ...
                            className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg px-2 py-1 transition-colors text-xs font-bold text-zinc-300"
                        >
                        ...
                        {isCurrencyMenuOpen && (
                            <div className="absolute top-full right-0 mt-2 w-24 bg-[#1C1C1E] border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[60]">
                                {AVAILABLE_CURRENCIES.map(curr => (
                                    <button 
                                        ...
                                        className={`w-full text-left px-3 py-2 text-xs font-bold hover:bg-zinc-800 flex justify-between items-center ${currentCurrency === curr ? 'text-blue-500 bg-blue-500/10' : 'text-zinc-400'}`}
                                    >
                                        {curr}
                ...
                <div className={`text-4xl md:text-5xl font-bold text-white mt-1 mb-1 tracking-tight transition-all duration-300 ${showBalance ? '' : 'blur-md select-none opacity-50'}`}>
                    {currencySymbol}{totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
                
                <div className="flex items-center gap-6 mt-6">
                    <div>
                        <span className="text-zinc-500 text-xs font-bold uppercase block mb-1">{t('24h Profit/Loss', '24h Ganancia/Pérdida')}</span>
                        <div className={`flex items-center gap-1 font-semibold transition-all duration-300 ${showBalance ? (dayChangeValue >= 0 ? 'text-green-400' : 'text-red-400') : 'blur-sm select-none opacity-50 text-zinc-400'}`}>
                            {showBalance && (dayChangeValue >= 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />)}
                            {currencySymbol}{Math.abs(dayChangeValue).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
                    </div>
                    <div className="w-px h-8 bg-zinc-800"></div>
                    <div>
                        <span className="text-zinc-500 text-xs font-bold uppercase block mb-1">{t('Total Profit', 'Ganancia Total')}</span>
                        <div className={`flex items-center gap-1 font-semibold transition-all duration-300 ${showBalance ? (totalProfit >= 0 ? 'text-green-400' : 'text-red-400') : 'blur-sm select-none opacity-50 text-zinc-400'}`}>
      ...
      <div className="px-4">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-zinc-200">{t('Your Assets', 'Tus Activos')}</h2>
            
            {/* INLINE PLUS BUTTON */}
            <button 
                onClick={onOpenAddModal}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-800 text-blue-400 hover:bg-blue-600 hover:text-white transition-all border border-zinc-700 hover:border-blue-500"
            >
