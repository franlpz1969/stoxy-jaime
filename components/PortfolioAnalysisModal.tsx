...
const PortfolioAnalysisModal: React.FC<PortfolioAnalysisModalProps> = ({
      ...
      <div className="my-8 overflow-hidden group/table">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b-2 border-gray-200 dark:border-zinc-800/40">
                {headers.map((h, i) => (
                  <th key={i} className="py-3 px-1 font-black text-gray-500 dark:text-zinc-500 uppercase text-[9px] tracking-[0.3em] group-hover/table:text-gray-700 dark:group-hover/table:text-zinc-300 transition-colors">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-900/30">
              {rows.map((row, i) => (
                <tr key={i} className="hover:bg-blue-50 dark:hover:bg-blue-500/[0.03] transition-colors group/row">
                  {row.map((cell, j) => (
                    <td key={j} className={`py-4 px-1 text-gray-700 dark:text-zinc-300 font-medium ${j > 0 ? 'font-mono text-xs text-blue-600 dark:text-blue-400/90' : 'text-sm'}`}>
                      {cell}
                    </td>
                  ))}
                </tr>
              ...
              {listItems.length > 0 ? (
                <ul className="space-y-4">
                  {listItems.map((item, i) => (
                    <li key={i} className="text-gray-700 dark:text-zinc-300 text-[13px] leading-relaxed flex gap-4 group">
                      <ChevronRight size={14} className="mt-1 text-gray-400 dark:text-zinc-700 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors shrink-0" />
                      <span className="group-hover:text-black dark:group-hover:text-white transition-colors">{item}</span>
                    </li>
                  ))}
                </ul>
              ) : (
                <div className="flex items-center gap-3 text-gray-500 dark:text-zinc-600 italic text-xs py-2">
                  <Info size={14} />
  ...
  const renderSectionContent = (id: string, content: string) => {
    ...
    return (
      <div className="text-gray-600 dark:text-zinc-300 text-base leading-loose space-y-5 font-medium pl-2">
        {content.split('\n').map((line, lidx) => {
          const trimmed = line.trim();
          if (trimmed.startsWith('-') || trimmed.startsWith('*')) {
            return (
              <div key={lidx} className="flex gap-4 items-start animate-in slide-in-from-left-2 duration-300 group">
                <div className="w-1.5 h-1.5 rounded-full bg-blue-600 mt-2.5 shrink-0 shadow-[0_0_10px_rgba(37,99,235,0.4)] group-hover:scale-125 transition-transform"></div>
                <span className="text-gray-700 dark:text-zinc-300 group-hover:text-black dark:group-hover:text-white transition-colors">{trimmed.substring(1).trim()}</span>
              </div>
            );
          }
          if (trimmed.startsWith('###')) {
            return (
              <h5 key={lidx} className="text-black dark:text-white font-black text-xs uppercase tracking-[0.2em] mt-12 mb-6 flex items-center gap-4">
          ...
          return <p key={lidx} className="text-gray-600 dark:text-zinc-400 first-letter:text-xl first-letter:font-bold first-letter:text-blue-600 dark:first-letter:text-blue-500 first-letter:mr-1">{trimmed}</p>;
        })}
      </div>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-0 bg-white/10 dark:bg-black/50 backdrop-blur-sm animate-in fade-in duration-700">

      {/* Immersive Dynamic Background */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-blue-500/10 dark:bg-blue-600/5 blur-[160px] rounded-full animate-pulse"></div>
        <div className="absolute bottom-[-20%] right-[-20%] w-[60%] h-[60%] bg-indigo-500/10 dark:bg-indigo-600/5 blur-[160px] rounded-full animate-pulse delay-1000"></div>
        {/* Scanline effect */}
          ...
          {isAnalyzing ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-12 animate-in fade-in zoom-in duration-1000">
              <div className="relative">
                <div className="w-48 h-48 border border-gray-100 dark:border-zinc-900 rounded-full flex items-center justify-center">
              ...
              <div className="space-y-6">
                <h3 className="text-gray-900 dark:text-white font-black text-4xl tracking-tighter">Procesando Tesis Inversora...</h3>
                <p className="text-gray-500 dark:text-zinc-600 text-xl max-w-lg mx-auto leading-relaxed font-bold italic tracking-tight">
            ...
            <div className="max-w-4xl mx-auto space-y-24 relative">
              ...
              {sections.length > 0 ? (
                sections.map((section, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-5 mb-10 group/title">
                      <div className="text-blue-600 dark:text-blue-500 p-3 bg-blue-50 dark:bg-blue-500/5 rounded-2xl border border-blue-200 dark:border-blue-500/10 group-hover/title:bg-blue-100 dark:group-hover/title:bg-blue-500/10 transition-colors">
                        {getIconForSection(section.id)}
                      </div>
                      <h4 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white tracking-tight uppercase tracking-[0.05em] group-hover/title:text-blue-600 dark:group-hover/title:text-blue-400 transition-colors">{section.title}</h4>
                    </div>
                ...
                <div className="text-gray-500 dark:text-zinc-500 text-xl leading-relaxed whitespace-pre-wrap font-bold italic py-20 text-center border border-dashed border-gray-200 dark:border-zinc-800 rounded-3xl">
                  Iniciando secuencia de análisis... <br />
                  <span className="text-xs font-normal mt-4 block text-gray-400 dark:text-zinc-700">Si el informe no aparece en 10 segundos, reintente la operación.</span>
                </div>
              )}

              {/* Footer Disclaimer - Elegant Typography */}
              <div className="pt-20 pb-32 flex flex-col items-center gap-10">
                <div className="p-8 rounded-[2.5rem] bg-gray-50 dark:bg-gradient-to-br dark:from-zinc-900/40 dark:to-black border border-gray-200 dark:border-zinc-800/40 flex gap-6 max-w-2xl shadow-sm dark:shadow-inner relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                    <ShieldAlert size={80} className="text-gray-900 dark:text-white" />
                  </div>
                  <AlertCircle size={28} className="text-amber-500 shrink-0 mt-1" />
                  <div className="space-y-3">
                    <p className="text-[12px] text-gray-500 dark:text-zinc-400 font-bold leading-relaxed uppercase tracking-widest">
                      Cláusula de Exención Institucional
                    </p>
                    <p className="text-[11px] text-gray-600 dark:text-zinc-600 font-medium leading-relaxed italic">
                ...
                <div className="text-gray-400 dark:text-zinc-800 text-[11px] font-black uppercase tracking-[0.6em] animate-pulse">
