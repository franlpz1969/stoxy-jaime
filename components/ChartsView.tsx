...
const ChartsView: React.FC<ChartsViewProps> = ({ portfolio }) => {
  ...
  // Heatmap State
  type HeatmapMode = 'normal' | 'sector' | 'country';
  ...
  const heatmapData = useMemo(() => {
    ...
    const items = portfolio.map((position) => {
      let shares = 0;
      ...
      return {
        ...
        country: position.stock.country || 'Unknown'
      };
    })
    ...
    // 3. Grouping Logic (Aggregate Mode)
    // We want to return BLOCKS representing the Groups, not children.
    const groups: Record<string, { name: string, value: number, weightedChangeSum: number, count: number }> = {};

    items.forEach(item => {
      const key = heatmapMode === 'sector' ? item.sector : item.country;
      if (!groups[key]) {
        groups[key] = { name: key, value: 0, weightedChangeSum: 0, count: 0 };
      }
      groups[key].value += item.value;
      groups[key].weightedChangeSum += (item.dayChange * item.value);
      groups[key].count += 1;
    });

    // Convert to array and calculate weighted average per group
    return Object.values(groups).map(g => ({
      name: g.name,
      value: g.value,
      dayChange: g.value > 0 ? g.weightedChangeSum / g.value : 0,
      isGroup: true,
      count: g.count,
      currency: 'USD' // Default for aggregated view
    })).sort((a, b) => b.value - a.value);
  ...
  const CustomHeatmapContent = (props: any) => {
    const { x, y, width, height, name, dayChange, isGroup, count } = props;

    // Create a unique ID for the clip path based on coordinates and name to avoid conflicts
    const clipId = `clip-${name}-${Math.round(x)}-${Math.round(y)}`;
    ...
    const fontSizeSymbol = Math.min(width / 6, height / 4, 18); // Slightly different scaling for potentially longer names
    const fontSizePercent = Math.min(width / 8, height / 6, 12);
    ...
    return (
      <g>
        ...
        <rect
          ...
          width={width}
          height={height}
          style={{ fill: color, stroke: '#000000', strokeWidth: 1, vectorEffect: 'non-scaling-stroke' }}
        />
        {showText && (
          <g clipPath={`url(#${clipId})`}>
            ...
            {isGroup && height > 60 && (
              <text
                x={x + width / 2}
                y={y + height / 2 + (fontSizeSymbol * 0.8) + 14}
                textAnchor="middle"
  ...
  const HeatmapTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      // If it's an aggregated group
      if (data.isGroup) {
        return (
          <div className="bg-zinc-950 border border-zinc-700 p-4 rounded-xl shadow-2xl min-w-[180px] z-50">
            <div className="flex justify-between items-center mb-2 gap-4">
              <span className="font-bold text-lg text-white">{data.name}</span>
              <span className={`font-bold px-2 py-0.5 rounded text-xs whitespace-nowrap ${safeDayChange >= 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                Avg: {safeDayChange > 0 ? '+' : ''}{safeDayChange.toFixed(2)}%
              </span>
            </div>
            <div className="space-y-1 text-sm text-zinc-400">
              <div className="flex justify-between"><span>Assets:</span><span className="text-white font-mono">{data.count} items</span></div>
              <div className="flex justify-between pt-2 border-t border-zinc-800 mt-1"><span>Total Value:</span><span className="text-white font-mono font-bold">${data.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
            </div>
          </div>
        );
      }

      // Normal single stock tooltip
        ...
        <div className="bg-zinc-950 border border-zinc-700 p-4 rounded-xl shadow-2xl min-w-[160px] z-50">
          ...
          <div className="space-y-1 text-sm text-zinc-400">
            ...
            {(data.sector || data.country) && (
              <div className="pt-2 mt-1 border-t border-zinc-800 text-xs text-zinc-500 flex gap-2">
  ...
  return (
    <div className="p-4 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 h-screen flex flex-col">
      ...
      <div className={`flex-1 bg-[#1C1C1E] border border-zinc-800 rounded-3xl p-1 relative shadow-lg flex flex-col ${chartType === 'allocation' || (chartType === 'heatmap') ? 'overflow-visible' : 'overflow-hidden'}`}>

        {/* Heatmap View */}
        {chartType === 'heatmap' && (
          <>
            {/* Header Container with Dropdown logic similar to Allocation */}
            <div className={`absolute top-4 left-6 z-20 pointer-events-none`}>
              <div className="pointer-events-auto">
                <h3 className="text-zinc-300 font-bold text-sm uppercase tracking-wider mb-1">Market Map</h3>
                <div className="relative inline-block">
                  <button
                    onClick={() => setIsHeatmapMenuOpen(!isHeatmapMenuOpen)}
                    className="flex items-center gap-1.5 bg-zinc-900/80 border border-zinc-800 px-2 py-1 rounded text-xs text-white font-bold hover:bg-zinc-800 transition-colors"
                  >
                    {heatmapMode === 'normal' ? 'Normal' : heatmapMode === 'sector' ? 'By Sector' : 'By Country'}
                    <ChevronDown size={12} className={`text-zinc-500 transition-transform ${isHeatmapMenuOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {isHeatmapMenuOpen && (
                    <div className="absolute top-full left-0 mt-2 w-32 bg-zinc-950 border border-zinc-800 rounded-lg shadow-xl overflow-hidden py-1 z-[60]">
                      <button
                        onClick={() => { setHeatmapMode('normal'); setIsHeatmapMenuOpen(false); }}
                        className="w-full text-left px-3 py-1.5 text-xs font-bold text-zinc-400 hover:text-white hover:bg-zinc-800 flex items-center justify-between"
                      >
                        Normal
                        {heatmapMode === 'normal' && <Check size={12} className="text-blue-500" />}
                      </button>
                      <button
                        onClick={() => { setHeatmapMode('sector'); setIsHeatmapMenuOpen(false); }}
                        className="w-full text-left px-3 py-1.5 text-xs font-bold text-zinc-400 hover:text-white hover:bg-zinc-800 flex items-center justify-between"
                      >
                        By Sector
                        {heatmapMode === 'sector' && <Check size={12} className="text-blue-500" />}
                      </button>
                      <button
                        onClick={() => { setHeatmapMode('country'); setIsHeatmapMenuOpen(false); }}
                        className="w-full text-left px-3 py-1.5 text-xs font-bold text-zinc-400 hover:text-white hover:bg-zinc-800 flex items-center justify-between"
                      >
                        By Country
                        {heatmapMode === 'country' && <Check size={12} className="text-blue-500" />}
                      </button>
                    </div>
                  )}
                  {/* Overlay for closing */}
                  {isHeatmapMenuOpen && (
                    <div className="fixed inset-0 z-40 bg-transparent" onClick={() => setIsHeatmapMenuOpen(false)} />
                  )}
                </div>
              </div>
            </div>
